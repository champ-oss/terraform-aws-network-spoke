...
module "internet_test" {
  enabled             = var.enabled
  source              = "github.com/champ-oss/terraform-aws-lambda.git?ref=v1.0.154-2280efb"
  enable_cw_event     = true
  enable_function_url = false
  enable_vpc          = true
  filename            = try(data.archive_file.this[0].output_path, "")
  git                 = "network-spoke"
  ...
  source_code_hash    = try(data.archive_file.this[0].output_base64sha256, "")
  timeout             = 15
  vpc_id              = module.this.vpc_id
  environment = {
    URL         = "api.seeip.org"
    SEARCH_DATA = try(join(",", [for _, value in module.hub[0].nat_gateway_attributes_by_az : value.public_ip]), "")
  }
}
